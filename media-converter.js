...
if (!ffmpegPath) {
  throw new Error("ServiceCall FFmpeg executable was not found.");
}
...
function getServiceCallTempDirectory() {
  const tempDirectory = path.join(os.tmpdir(), "ServiceCall");
...
function deleteFileSafely(filePath) {
  ...
  try {
    ...
    console.warn("Unable to delete temporary ServiceCall file:", error.message);
  }
}
...
function normalizeRecordingBuffer(webmBuffer) {
  if (!webmBuffer) {
    throw new Error("ServiceCall recording data was not provided.");
  }

  const inputBuffer = Buffer.isBuffer(webmBuffer)
    ? webmBuffer
    : Buffer.from(webmBuffer);

  if (inputBuffer.length <= 0) {
    throw new Error("ServiceCall recording is empty.");
  }
...
async function convertWebmToMp3(webmBuffer) {
  const inputBuffer = normalizeRecordingBuffer(webmBuffer);

  const tempDirectory = getServiceCallTempDirectory();

  const tempId = createTempId();

  const inputPath = path.join(tempDirectory, tempId + ".webm");

  const outputPath = path.join(tempDirectory, tempId + ".mp3");
  ...
  try {
    await new Promise((resolve, reject) => {
      ffmpeg(inputPath)
        ...
        .on("start", () => {
          console.log("ServiceCall MP3 conversion started.");
        })
    ...
    if (mp3Buffer.length <= 0) {
      throw new Error("Converted ServiceCall MP3 is empty.");
    }
...
async function convertWebmToMp4(webmBuffer) {
  ...
  const outputPath = path.join(tempDirectory, tempId + ".mp4");
  ...
  try {
    await new Promise((resolve, reject) => {
      ffmpeg(inputPath)
        ...
        .on("start", () => {
          console.log("ServiceCall MP4 conversion started.");
        })
    ...
    if (mp4Buffer.length <= 0) {
      throw new Error("Converted ServiceCall MP4 is empty.");
    }
...
module.exports = {
  ...
  convertWebmToMp4: convertWebmToMp4,
};
