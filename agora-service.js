 ...
 */
const remoteScreenVideoTracks = new Map();
...
function createClient() {
    ...
    async (user, mediaType) => {
      try {
        ...
        if (mediaType === "audio") {
          ...
          console.log("ServiceCall remote audio playing:", user.uid);
        ...
        if (mediaType === "video") {
          ...
          remoteScreenVideoTracks.set(String(user.uid), user.videoTrack);

          console.log("ServiceCall remote screen track received:", user.uid);
        ...
        console.error("Unable to subscribe to remote ServiceCall user:", error);
      }
    },
  );
  ...
  client.on(
    ...
    (user, mediaType) => {
      ...
      if (mediaType === "audio") {
        ...
        console.log("ServiceCall remote audio track removed:", user.uid);
      }
      ...
      if (mediaType === "video") {
        ...
        const remoteTrack = remoteScreenVideoTracks.get(uid);
        ...
        remoteScreenVideoTracks.delete(uid);

        console.log("ServiceCall remote screen track removed:", user.uid);
      ...
      console.log("ServiceCall remote user unpublished:", user.uid, mediaType);
    },
  );
  ...
  client.on(
    ...
    (user) => {
      ...
      const remoteVideoTrack = remoteScreenVideoTracks.get(uid);
      ...
      console.log("ServiceCall remote user left:", user.uid);
  ...
  client.on(
    ...
    async () => {
      console.log("ServiceCall Agora token will expire soon.");

      if (tokenRenewalInProgress) {
        console.log("ServiceCall token renewal is already in progress.");

        return;
      }

      if (typeof tokenRenewalHandler !== "function") {
        console.error("ServiceCall token renewal handler is unavailable.");
      ...
      try {
        ...
        if (!newToken) {
          throw new Error("A renewed Agora token was not returned.");
        }
        ...
        console.log("ServiceCall Agora token renewed successfully.");
      } catch (error) {
        console.error("ServiceCall Agora token renewal failed:", error);
      } finally {
...
async function joinAudioCall(config) {
  if (joined) {
    console.log("ServiceCall is already connected to Agora.");
  ...
  if (
    ...
    throw new Error("Agora media credentials are incomplete.");
  }

  tokenRenewalHandler =
    typeof config.renewToken === "function" ? config.renewToken : null;
  ...
  if (
    ...
    throw new Error("Agora participant UID is invalid.");
  }
  ...
  try {
    ...
    console.log("ServiceCall microphone published successfully.");
    ...
    console.error("Unable to join ServiceCall audio:", error);
...
async function setMuted(shouldMute) {
  if (!localAudioTrack) {
    throw new Error("ServiceCall microphone is not active.");
  }
...
async function startScreenShare(screenTrack) {
  if (!joined || !client) {
    throw new Error("ServiceCall must be connected before sharing a screen.");
  }
  ...
  if (!screenTrack) {
    throw new Error("A screen video track was not provided.");
  }

  try {
    localScreenVideoTrack = screenTrack;

    await client.publish([localScreenVideoTrack]);

    screenSharing = true;

    console.log("ServiceCall screen published successfully.");
    ...
    console.error("Unable to publish ServiceCall screen:", error);
  ...
  try {
    if (client) {
      try {
        ...
        console.error("Unable to unpublish ServiceCall screen:", error);
      }
    }
    ...
    console.log("ServiceCall screen sharing stopped.");
  }
...
function getRemoteAudioTracks() {
  return Array.from(remoteAudioTracks.values());
}
...
function getRemoteScreenVideoTracks() {
  return Array.from(remoteScreenVideoTracks.values());
}
...
async function leaveAudioCall() {
  try {
     ...
     */
    remoteScreenVideoTracks.forEach((track) => {
      try {
        track.stop();
      } catch (error) {
        // Ignore playback stop error.
    ...
    console.log("ServiceCall left Agora media.");
  }
...
window.ServiceCallAgora = {
  ...
  setRemoteScreenHandler: setRemoteScreenHandler,
  ...
  getRemoteAudioTrackCount: getRemoteAudioTrackCount,

  getLocalScreenVideoTrack: getLocalScreenVideoTrack,

  getRemoteScreenVideoTracks: getRemoteScreenVideoTracks,
};

async function createScreenVideoTrack(mediaStreamTrack) {
  if (!mediaStreamTrack) {
    throw new Error("Screen MediaStreamTrack was not provided.");
  }
